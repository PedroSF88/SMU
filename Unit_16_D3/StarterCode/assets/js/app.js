...
function xScale(povData, chosenXAxis) {
  ...
  var xLinearScale = d3.scaleLinear()
    .domain([d3.min(povData, d => d[chosenXAxis])-1,
      d3.max(povData, d => d[chosenXAxis])
    ])
    .range([0, width]);
...
d3.csv("/assets/data/data.csv").then(function(povData, err) {
  ...
  var circlesGroup = chartGroup.selectAll("circle")
    ...
    var texts = chartGroup.selectAll(".text").append("g")
    .data(povData)
    .enter()
    .append("text")
    ...
    var toolTip = d3.tip()
        ...
        .offset([80, -60])
        .html(function(d) {
            return (`${d.state}<br>Poverty: ${d.poverty}%<br>Healthcare: ${d.healthcare}%<br> Median smokes: ${d.smokes}%`);
        });
